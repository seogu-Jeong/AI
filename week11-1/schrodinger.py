...
def plot_results(x, V, energies, psis, title, filename, x_lim=None):
    ...
    for i, (E, psi) in enumerate(zip(energies, psis)):
        ...
        ax.plot(x, E + scale * psi,  color=colors_psi[i],  linewidth=1.5,
                label='$\\psi_n$ (blue)' if i == 0 else None)
        ax.plot(x, E + scale * prob, color=colors_prob[i], linewidth=1.5,
                linestyle='-.', label='$|\\psi|^2$ (red)' if i == 0 else None)
...
def main():
    ...
    x_inf = np.linspace(0, L, N + 2)[1:-1]
    V_inf = potential_infinite_well(x_inf)
    E_inf, psi_inf = solve(x_inf, V_inf, N_STATES)

    analytical_inf = np.array([(n**2 * np.pi**2) / (2 * L**2) for n in range(1, N_STATES + 1)])
    results_ok &= print_comparison("Infinite Well: E_n = n²π²/(2L²)", E_inf, analytical_inf, N_STATES)

    print("\nNormalization check:")
    results_ok &= check_normalization(psi_inf, x_inf)

    print("\nNode count check:")
    for i, psi in enumerate(psi_inf):
        nodes = count_nodes(psi)
        status = "OK" if nodes == i else "FAIL"
        print(f"  state {i} (n={i+1}): nodes={nodes}, expected={i} [{status}]")
        if nodes != i:
            results_ok = False
    ...
    results_ok &= print_comparison("Harmonic: E_n = (n+½)ω  (ω=1, 0-based n)", E_harm, analytical_harm, N_STATES)

    print("\nNormalization check:")
    results_ok &= check_normalization(psi_harm, x_harm)

    print("\nNode count check:")
    for i, psi in enumerate(psi_harm):
        nodes = count_nodes(psi)
        status = "OK" if nodes == i else "FAIL"
        print(f"  state {i}: nodes={nodes}, expected={i} [{status}]")
        if nodes != i:
            results_ok = False
    ...
    E_fin_all, psi_fin_all = solve(x_fin, V_fin, 20)
    ...
    if len(x_right) > 10:
        ...
        ok_decay = slope < 0
        print(f"  Exponential decay slope: {slope:.3f}  ({'OK' if ok_decay else 'FAIL'})")
        results_ok &= ok_decay

    n_plot = min(N_STATES, n_found)
    print("\nNormalization check:")
    results_ok &= check_normalization(psi_bound[:n_plot], x_fin)
    ...
    plot_results(x_fin, V_fin, E_bound[:n_plot], psi_bound[:n_plot],
                 ...
                 "finite_well.png", x_lim=(-4 * a, 4 * a))

    print("\n" + "="*60)
    print("ALL CRITERIA PASSED ✓" if results_ok else "SOME CRITERIA FAILED")
    print("="*60)
